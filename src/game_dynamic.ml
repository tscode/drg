...
let check_auth card event world =
  match Game_auth.auth card world with
  | Error msg -> error msg
  | Ok id -> match id, event with
    | -1, _ -> Ok id
    | _, Event.Impl _ -> error ("Only god can implement modifications")
    | _, _ -> Ok id

let evolve card event world =
  match check_auth card event world with
  | Error msg -> (world, error [msg])
  | Ok id ->
    let info = event_info id world in
    match event with
...
let evolve_exn card event world =
  match evolve card event world with
  | (w, Ok _) -> w
  | (w, Error msg) -> raise (Event_error (Bytes.concat "\n" msg))
